  ...
  const deleteDialog = (id: number, name: string) => (
    <AlertDialog>
      <AlertDialogTrigger asChild>
        <Button variant="ghost" size="icon" className="text-destructive h-10 w-10" data-testid={`button-delete-machine-${id}`}>
            ...
            >
              <div className="px-4 pt-4 pb-3">
                ...
                <div className="flex gap-4 mt-3">
                  <div>
                    <div className="text-xs font-medium" style={{ color: "hsl(var(--muted-foreground))" }}>Hourly Rate</div>
                    <div className="font-mono font-semibold text-sm">£{machine.hourlyRate.toFixed(2)}/hr</div>
                  </div>
                  <div>
                    <div className="text-xs font-medium" style={{ color: "hsl(var(--muted-foreground))" }}>Setup Rate</div>
                    <div className="font-mono font-semibold text-sm">£{machine.setupRate.toFixed(2)}/hr</div>
                  </div>
                </div>
              </div>
              <div
              ...
              >
                <Link href={`/machines/${machine.id}/edit`}>
                  <Button variant="ghost" size="sm" className="h-10 px-3 text-xs gap-1.5" data-testid={`button-edit-machine-${machine.id}`}>
            ...
            {isLoading ? (
              ...
              machines?.map((machine) => (
                <TableRow key={machine.id} data-testid={`row-machine-${machine.id}`}>
                  ...
                  <TableCell className="font-mono">£{machine.hourlyRate.toFixed(2)}</TableCell>
                  <TableCell className="font-mono">£{machine.setupRate.toFixed(2)}</TableCell>
                  <TableCell>
                    <Switch
                      checked={machine.active}
                      onCheckedChange={(checked) => handleToggleActive(machine.id, checked)}
