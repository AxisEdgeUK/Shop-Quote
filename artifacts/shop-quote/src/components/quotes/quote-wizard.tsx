            ...
            <div className="space-y-4">
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Show alternative pricing for different order quantities on
                    the PDF. Select the quantities to include.
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-3">
                    ...
                    {[5, 10, 25, 50, 100, 250, 500].map((qty) => {
                      ...
                      return (
                        <button
                          ...
                          onClick={() => {
                            const next = active
                              ? current.filter((q) => q !== qty)
                              : [...current, qty].sort((a, b) => a - b);
                            form.setValue(
                              "priceBreakQtys",
                              JSON.stringify(next),
                            );
                  ...
                  {(() => {
                    const qtys: number[] = (() => {
                      try {
                        return JSON.parse(form.watch("priceBreakQtys") || "[]");
                      } catch {
                        return [];
                      }
                    })();
                    return qtys.length > 0 ? (
                      <p className="text-xs text-muted-foreground">
                        Price break columns will appear on the PDF for:{" "}
                        <strong>
                          {qtys.map((q) => `${q} off`).join(", ")}
                        </strong>
                      </p>
                    ) : (
                      <p className="text-xs text-muted-foreground">
                        No price breaks selected — quote will show a single
                        price per line item.
                      </p>
                    );
                  })()}
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        >
                          ...
                          <SelectContent>
                            {["Loose", "Standard", "Tight", "Critical"].map(
                              (v) => (
                                <SelectItem key={v} value={v}>
                                  {v}
                                </SelectItem>
                      ...
                      >
                        ...
                        {!completedCustomer.email && (
                          <p className="text-xs text-amber-500">
                            ...
                            in Customers to auto-fill To: field.
                          </p>
                        )}
                        <p className="text-xs text-muted-foreground">
